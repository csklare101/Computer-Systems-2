 ...
 *---		whatIsClient.c						---*
 *---									---*
 *---	    This file defines a C program that gets potential Unix 	---*
 *---	commands from the user, and sends them to a server via a 	---*
 *---	socket, waits for a reply to a "whatis" call, and outputs the	---*
...
//	Compile with:
//	$ gcc whatIsClient.c -o whatIsClient
				...
				)
{
  ...
  char	fullBuffer[BUFFER_LEN+1];
  char*	charBuffer	= fullBuffer+sizeof(int);
  int	charBufferLen	= sizeof(fullBuffer)-sizeof(int);
  ...
  while  (shouldContinue)
  {
    ...
    do
    {
      printf("Unix command or interest or \"%s\" to quit: ",QUIT_CMD);
      fgets(charBuffer,charBufferLen,stdin);
      cPtr	= strchr(charBuffer,'\n');
	...
	*cPtr	= '\0';
    ...
    if  (strcasecmp(charBuffer,QUIT_CMD) == 0)
    {
      shouldContinue	= 0;
    }

    int	numBytes	= strlen(charBuffer)+1;

    printf("Sending \"%d\"\n",numBytes);
    printf("Sending \"%s\"\n",charBuffer);
    *(int*)fullBuffer	= htonl(numBytes);
    write(socketFd,fullBuffer,numBytes+sizeof(int));

    if  (shouldContinue)
    {
      printf("Reading...\n");
      numBytes = read (socketFd,charBuffer,charBufferLen);

      if  (numBytes > 0)
        charBuffer[numBytes] = '\0';
